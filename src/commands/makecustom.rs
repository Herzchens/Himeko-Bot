...
/// Chia team ngẫu nhiên cho Valorant Custom
#[poise::command(slash_command)]
pub async fn makecustom(
    ctx: Context<'_>,
    #[description = "Tự động lấy người trong Voice Channel hiện tại (Mặc định: true)"]
    ...
    let maps = [
        "Abyss", "Lotus", "Sunset", "Breeze", "Icebox", "Fracture", "Pearl", "Ascent", "Haven",
        "Bind", "Split",
    ];
    let map = maps.choose(&mut rng()).expect("maps must not be empty");

    let mut response =
        format!("**VALORANT CUSTOM MATCH** \n **Map**: {map}\n\n **Phe Tấn Công (Attackers)**:\n");
